...
parser.add_argument("--version", default='test_coco_nohead_ground_truth_max',type=str, help="gpu device id (if applicable)")

parser.add_argument("--parameter_embedding_folder", default='parameters/image/coco/val',type=str, help="gpu device id (if applicable)")
parser.add_argument("--lora_dir", default='/home/u2021010261/data/yx/Mobile-Search-Engine-main/.checkpoints/lora/coco/trunk/ratio3/e12/32',type=str, help="gpu device id (if applicable)")
parser.add_argument("--model_parameter", default='parameters/image/coco/model/image_S={S}_val_v1.pth',type=str, help="gpu device id (if applicable)")


parser.add_argument("--coarse_embedding_path", default='embeddings_{N}_trunk_lora.pth',type=str, help="gpu device id (if applicable)")
parser.add_argument("--fine_model_embeddings", default='embeddings_{full_layer}_trunk_lora.pth',type=str, help="gpu device id (if applicable)")
parser.add_argument("--text_embeddings_dir", default='text_embeddings_trunk_lora_{version}.pt',type=str, help="gpu device id (if applicable)")
parser.add_argument("--ground_truth", default='results/coco_lora_val_nohead/R10/layers.txt',type=str, help="gpu device id (if applicable)")
parser.add_argument("--csv_file_path", default='e2e_coco_lora_{version}.csv',type=str, help="gpu device id (if applicable)")
...
model_parameter=model_parameter.format(S=S)

coarse_embedding_path=args.coarse_embedding_path
coarse_embedding_path=parameter_embedding_folder+'/'+coarse_embedding_path.format(N=N)

fine_model_embeddings=args.fine_model_embeddings
fine_model_embeddings=parameter_embedding_folder+'/'+fine_model_embeddings.format(full_layer=full_layer)

text_embeddings_dir=args.text_embeddings_dir
text_embeddings_dir=parameter_embedding_folder+'/'+text_embeddings_dir.format(version=version)

csv_file_path=args.csv_file_path
csv_file_path=csv_file_path.format(version=version)
