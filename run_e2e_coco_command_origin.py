...
for i, (N, S, Q) in enumerate(combinations):
    ...
    command = f"python coco_mobile-search_engine_lora_origin.py --S {S} --N {N} --Q {Q} --device {device} > {log_file} 2>&1"
...
with open('commands_origin.txt', 'w') as f:
    json.dump(commands, f)
    
print("Commands have been saved to 'commands_origin.txt'")
