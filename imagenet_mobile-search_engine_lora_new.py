...
shortlist_path=f"{parameter_embedding_folder}/shortlist/shortlist_data_{N}_{Q}_S={S}_v{version}.pkl"
    ...
    with torch.no_grad():
        ...
        for batch_idx, (x, target) in enumerate(test_dl):
            ...
            if batch_idx==len(test_dl)-1:
                match_value = coarse_embedding_dynamic[ModalityType.VISION][batch_idx*batch_size:]@text_embeddings.T 
            else:
        ...
        for batch_idx, (x, target) in enumerate(test_dl):
            ...
            for k, top_indices, item in zip(K_list, top_indices_list, K_image_correct_list):
                ...
                for i,row in enumerate(top_indices):
                    ...
                    list_item=[]
                    for item_r in row:
                        list.append(test_ds.text_list[item_r])
...
with torch.no_grad():
    ...
    for batch_idx, (x, target) in enumerate(test_dl_final):
        ...
        for item in shortlist_item[f'K={Q}'][batch_idx*batch_size]:
            ...
            if embeddings_TEXT:
                embeddings_TEXT[ModalityType.TEXT]=torch.cat([embeddings_TEXT[ModalityType.TEXT],cur_text_embeddings.unsqueeze(0).to(embeddings_TEXT[ModalityType.TEXT].device)],dim=0)
            else:
