...
imagebind_targets=torch.load(imagebind_target_path, map_location=device)
...
if os.path.exists(coarse_embedding_dynamic_path):
    ...
    with torch.no_grad():
        checkpoint = torch.load(coarse_embedding_dynamic_path, map_location=device)
        # 获取模型参数和张量
...
fine_model = imagebind_model.imagebind_huge(pretrained=True, text_num_blocks=QL)
logging.info(f'text layer num is {fine_model.modality_trunks["text"].blocks}')
if lora_dir != None:
    ...
    if show_dynamic_embedding_accuracy:
        ...
        with torch.no_grad():
            ...
            for batch_idx, (_, x, image_name) in enumerate(test_dl):
                ...
                logging.info(text_embeddings)
                logging.info(coarse_embedding_dynamic[ModalityType.VISION])
        ...
        # # 指定CSV文件路径
        csv_file_path = f'ground_truth_lora_{version}_speculative.csv'
