...
for i in topk_list:
    if i%(worker_num-1)==0 and i!=0:
        command = f"python get_layers_harsmart.py --S {i} " # stop to run this command
    else:
        command = f"python get_layers_harsmart.py --S {i}" # put it in the backend and run the next one concurrently
    logging.info(f"Running command: {command}")
    os.system(command)
