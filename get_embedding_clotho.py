...
embedding_dir=f'{embedding_dir}/embeddings_{a_block}.pth'
...
with torch.no_grad():
    ...
    csv_file_path='/home/u2021010261/data/yx/Mobile-Search-Engine-main/.datasets/data/clotho_csv_files/clotho_captions_evaluation.csv'
    data_dir='/home/u2021010261/data/yx/Mobile-Search-Engine-main/.datasets/data/clotho_audio_files/evaluation'
    ...
    for batch_idx, (x, target) in enumerate(test_dl):
        inputs = {
            ModalityType.AUDIO: data.load_and_transform_audio_data(x,device=device),
        }
        # embeddings = model(inputs)
        current_embeddings = model(inputs)[ModalityType.AUDIO]
        
        if embeddings:
            embeddings[ModalityType.AUDIO] = torch.cat([embeddings[ModalityType.AUDIO], current_embeddings], dim=0)
        else:
            embeddings[ModalityType.AUDIO] = current_embeddings
    ...
    embeddings[ModalityType.AUDIO] = embeddings[ModalityType.AUDIO].to(device)
