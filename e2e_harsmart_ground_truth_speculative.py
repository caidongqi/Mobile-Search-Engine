...
parser.add_argument("--Q", default=10,type=int, help="Grain for predict model, larger S, smaller average predicted layer")
...
parser.add_argument("--version", default='harsmart_lora_val_without_head',type=str, help="gpu device id (if applicable)")
parser.add_argument("--parameter_embedding_folder", default='cuda:0',type=str)
parser.add_argument("--QL", default=1,type=int, help="Layer for building query embedding")
...
QL = args.QL # query_layer
# N=2
# Q=100

full_layer=6 #audio:12 image:32
device = args.device if torch.cuda.is_available() else "cpu"
parameter_embedding_folder='/home/u2021010261/share/pc/Mobile-Search-Engine/parameters/image/harsmart/val/without_head'
...
# 下面的三个名字，跑的时候尽量改一下
# dynamic embeddings
coarse_embedding_dynamic_path=f'{parameter_embedding_folder}/dynamic/S={S}_v{version}.pth'

#save layers
#layers_path=f"/home/u2021010261/share/pc/Mobile-Search-Engine/parameters/image/coco/layers/N={N}_S={S}_v{version}.pkl"
shortlist_path=f"{parameter_embedding_folder}/shortlist/shortlist_data_S={S}_v{version}.pkl" # Different Q could share the same shortlist
...
if os.path.exists(coarse_embedding_dynamic_path):
    ...
    with torch.no_grad():
        ...
        # 获取模型参数和张量
        coarse_embedding_dynamic[ModalityType.IMU]= checkpoint['vision_embeddings']
    ...
    with torch.no_grad():
        for i in range(len(layers)):
            ...
            if os.path.exists(current_coarse_embedding_dynamic_path):
                ...
                if coarse_embedding_dynamic:
                    coarse_embedding_dynamic[ModalityType.IMU] = torch.cat([coarse_embedding_dynamic[ModalityType.IMU], current_embeddings.unsqueeze(0).to(coarse_embedding_dynamic[ModalityType.IMU].device)], dim=0)
                else:
                    coarse_embedding_dynamic[ModalityType.IMU] = current_embeddings.unsqueeze(0)
        ...
        torch.save({
                'vision_embeddings': coarse_embedding_dynamic[ModalityType.IMU]
...
all_text_embeddings=torch.load(f'{parameter_embedding_folder}/embeddings_{QL}.pth',map_location=device)['vision_embeddings']
...
if os.path.exists(shortlist_path):
    ...
    if show_dynamic_embedding_accuracy:
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x, target) in enumerate(test_dl):
                ...
                text_embeddings=torch.cat([all_text_embeddings[i].unsqueeze(dim=0) for i in target.tolist()],dim=0)

                match_value = text_embeddings@coarse_embedding_dynamic[ModalityType.IMU].T 
        ...
        # # 指定CSV文件路径
        csv_file_path = f'e2e_harsmart_lora_{version}.csv'
    ...
    with torch.no_grad():
        ...
        for batch_idx, (x, target) in enumerate(test_dl):
            ...
            # print("------------------")
            # print(coarse_embedding_dynamic[ModalityType.IMU])
            match_value = text_embeddings@coarse_embedding_dynamic[ModalityType.IMU].T 
            ...
            for k, top_indices, item in zip(K_list, top_indices_list, K_caption_correct_list):
                ...
                for i,row in enumerate(top_indices):
                    ...
                    for item_r in row:
                        list_value.append(coarse_embedding_dynamic['imu'][item_r.item()])
    ...
    # # 指定CSV文件路径
    csv_file_path = f'e2e_harsmart_lora_{version}_speculative.csv'
...
with torch.no_grad():
    ...
    for batch_idx, (x, target) in enumerate(test_dl):
        ...
        predicted=torch.Tensor([shortlist_item[f'K={Q}'][batch_idx*batch_size][predicted]]) # 把0，1，3...的相对index转化为shortlist里面对应的真实数据的绝对index
