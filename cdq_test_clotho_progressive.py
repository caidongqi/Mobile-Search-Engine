...
def run_inference():
    ...
    # TODO: could save the embeddings_all here, and start from step 2. to debug the reduced_list.

    start_event.record()
    predicted = torch.max(new_model(embeddings_all), 1)[1]
    end_event.record()
    torch.cuda.synchronize()
    elapsed_time = start_event.elapsed_time(end_event)
    logging.info(f"Elapsed prediction time: {elapsed_time}ms")



    # Step 2: we sort the most confident ee and batch them accoradingly. ee = 1 and predict the ee of all samples (todo)
    # Step 3: We compute the second layer of the rest samples and predict their ee. Repeat it until all samples are processed. 
    groups = grouping(predicted)

    with open(f"./tmp/groups_dict_{ee}", "w") as json_file:
        groups_json = json.dumps(dict(groups), indent=4)
        json_file.write(groups_json)

    logging.info(f"Average predicted layer: {cal_avg_layer(groups)}")

    # 计算每个组的嵌入
    embeddings_by_group = {}
    re_enter = ee
    for ee_new, indices in groups.items():
        # 确定批处理大小：最小值是32和组的大小
        max_power_of_two = int(math.log2(len(indices)))
        batch_size_new = min(32, 2 ** max_power_of_two)
        
        # 用Subset根据索引创建数据集子集
        subset_ds = Subset(audioDataset, indices)
        
        # 计算嵌入
        embeddings = embedding(subset_ds, ee_new, batch_size_new, re_enter)
        embeddings_by_group[ee_new] = embeddings
