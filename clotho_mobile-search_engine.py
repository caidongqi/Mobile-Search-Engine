...
# # 1.存储n层 n=1
# N=1
embedding_folder=f'parameters/audio/trunks+post/embeddings_{N}.pth'
...
layers_folder=f"parameters/layers/layers_{N}.pkl"
...
if os.path.exists(embedding_dynamic_folder):
    embedding_dynamic={}
    with torch.no_grad():
            checkpoint = torch.load(f'parameters/dynamic/audio/embeddings_{N}.pth')
            # 获取模型参数和张量
    ...
    with torch.no_grad():
        ...
        torch.save({
            ...
            }, f'parameters/dynamic/audio/embeddings_{N}.pth')
...
# counts_r1=np.array([])
# counts_r10=np.array([])
# shortlist_folder=f"parameters/shortlist/shortlist_data_{N}.pkl"
...
#             # 假设你有一个文件路径用于保存数据
#             file_path = f"parameters/shortlist/shortlist_data_{N}"
