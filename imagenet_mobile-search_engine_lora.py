...
from models.mlp import MyModel,MyModel3
...
parser.add_argument("--model", default='model',type=int, help="Number of S")
...
test_ds1 = ImageNetDataset(datadir=imagenet_datadir, split=split, transform=data_transform)
    #test_dl1=DataLoader(dataset=test_ds1, batch_size=64, shuffle=False, drop_last=False, num_workers=4, pin_memory=True, persistent_workers=True)
test_dl1 = DataLoader(dataset=test_ds1, batch_size=1, shuffle=False, drop_last=False, num_workers=4, pin_memory=False, persistent_workers=True)
num_samples = 2500
batch_size=32
...
test_dl2=test_dl
#embedding path
parameter_embedding_folder=f'parameters/image/lora/trunks_{split}/'
...
if split=="train":
    embedding_folder=f'{parameter_embedding_folder}embeddings_{N}_train.pth' 
else:
    embedding_folder=f'{parameter_embedding_folder}embeddings_{N}.pth'
embedding_dynamic_folder=f'{parameter_embedding_folder}dynamic/{m}_N={N}_S={S}_v{version}.pth'
#save layers
layers_folder=f"{parameter_embedding_folder}layers/{m}_N={N}_S={S}_v{version}.pkl"


shortlist_folder=f"{parameter_embedding_folder}shortlist/shortlist_{m}_{N}_{K}_S={S}_v{version}.pkl"
if split=='train':
    fine_model_embeddings=f'{parameter_embedding_folder}embeddings_{full_layer}_train.pth'
else:
...
if m=='model':
    ...
    input_size = N  # Assuming input size
    outputs_label=32
    predict_model = MyModel3(input_size, outputs_label)  # Input shape modified to match the input size
    predict_model.to(device)
    # 加载已保存的模型参数
    predict_model.load_state_dict(torch.load(model_parameter,map_location=device))
    models = []
    ...
    if os.path.exists(layers_folder):
        ...
        for i in range(1, N + 1):
            if split=="train":
                embeddings_dict[str(i)] = torch.load(f'{parameter_embedding_folder}embeddings_{N}_train.pth')['audio_embeddings']
            else:
                embeddings_dict[str(i)] = torch.load(f'{parameter_embedding_folder}embeddings_train.pth')['audio_embeddings']
        ...
        outputs = predict_model(torch.tensor(concatenated_embeddings).float().to(device))
        _, predicted = torch.max(outputs, 1)
        layers= predicted.tolist()
        # for embedding_item in embeddings[ModalityType.VISION]:       
        #         embedding_item=embedding_item.to(device)
        #         layer=predict_model(embedding_item.float())
        #         _, layer1 = torch.max(layer, 0)
...
if os.path.exists(embedding_dynamic_folder):
    ...
    with torch.no_grad():
        for i in range(len(layers)):
                ...
                if split == "train":
                    parameter_embedding_folder1=parameter_embedding_folder+f'embeddings_{layers[i]}_train.pth'
                else:
