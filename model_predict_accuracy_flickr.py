...
parser.add_argument("--S", default=1,type=int, help="Number of S")
...
import os
layer_num = 32
root=f"parameters/image/flickr8k/val"
embeddings_dict = {}
...
layers = np.loadtxt(f'results/flickr8k_lora_val_nohead/R{S}/layers.txt')  
...
with torch.no_grad():
    ...
    # 将结果写入CSV文件
    with open(f'model_flickr_lora_val_N_S_min.csv', 'a', newline='') as csvfile:
    ...
    layer_means = [layer_dict[key].cpu() for key in layer_keys]
    np.savetxt('model_accuracies.txt',accuracies, fmt='%.3f')
    np.savetxt('layer_means.txt',layer_means, fmt='%.3f')
