...
parser.add_argument("--version", default='criterion_flicker',type=str, help="gpu device id (if applicable)")
parser.add_argument("--criterion", default=30,type=int, help="Fine grained embedding scope after query")
...
text_embeddings_dir = f'{parameter_embedding_folder}/text_embeddings_flickr.pt'

# 下面的三个名字，跑的时候尽量改一下
# dynamic embeddings
coarse_embedding_dynamic_path=f'{parameter_embedding_folder}/dynamic/N={N}_S={S}_c={criterion}_v{version}.pth'
...
ground_truth=f'results/flickr8k_confidence/c={criterion}_layers.txt'
...
if os.path.exists(shortlist_path):
    ...
    if show_dynamic_embedding_accuracy:
        ...
        results_dynamic.append(criterion)
