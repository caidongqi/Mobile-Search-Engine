...
parser.add_argument("--Q", default=20,type=int, help="Grain for predict model, larger S, smaller average predicted layer")
parser.add_argument("--S", default=300,type=int, help="Grain for predict model, larger S, smaller average predicted layer")
...
parser.add_argument("--version", default='clotho_head',type=str, help="gpu device id (if applicable)")
parser.add_argument("--parameter_embedding_folder", default='parameters/audio/clotho_val/with_head',type=str)
...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    handlers=[logging.FileHandler(f'logs/e2e/ground_truth_S={S}_{formatted_time}.log')], 
                    force=True)

full_layer=12 #audio:12 image:32
device = args.device if torch.cuda.is_available() else "cpu"
parameter_embedding_folder=args.parameter_embedding_folder
fine_model_embeddings = f'{parameter_embedding_folder}/embeddings_{full_layer}.pth'
text_embeddings_dir = f'{parameter_embedding_folder}/text_embeddings_S={S}_{version}.pt'

# 下面的三个名字，跑的时候尽量改一下
# dynamic embeddings
coarse_embedding_dynamic_path=f'{parameter_embedding_folder}/dynamic'
if not os.path.exists(coarse_embedding_dynamic_path):
    os.makedirs(coarse_embedding_dynamic_path, exist_ok=True)
coarse_embedding_dynamic_path=f'{coarse_embedding_dynamic_path}/S={S}_v{version}.pth'


#save layers
#layers_path=f"/home/u2021010261/share/pc/Mobile-Search-Engine/parameters/image/coco/layers/N={N}_S={S}_v{version}.pkl"
shortlist_path=f"{parameter_embedding_folder}/shortlist" # Different Q could share the same shortlist
if not os.path.exists(shortlist_path):
    os.makedirs(shortlist_path, exist_ok=True)
shortlist_path=f'{shortlist_path}/shortlist_data_S={S}_v{version}.pkl'
...
ground_truth=f'results/{version}/R{S}/layers.txt'
...
if os.path.exists(coarse_embedding_dynamic_path):
    ...
    with torch.no_grad():
        ...
        # 获取模型参数和张量
        coarse_embedding_dynamic[ModalityType.AUDIO]= checkpoint['audio_embeddings']
    ...
    with torch.no_grad():
        for i in range(len(layers)):
            ...
            if os.path.exists(current_coarse_embedding_dynamic_path):
                ...
                if coarse_embedding_dynamic:
                    coarse_embedding_dynamic[ModalityType.AUDIO] = torch.cat([coarse_embedding_dynamic[ModalityType.AUDIO], current_embeddings.unsqueeze(0).to(coarse_embedding_dynamic[ModalityType.AUDIO].device)], dim=0)
                else:
                    coarse_embedding_dynamic[ModalityType.AUDIO] = current_embeddings.unsqueeze(0)
        ...
        torch.save({
                'audio_embeddings': coarse_embedding_dynamic[ModalityType.AUDIO]
...
if not os.path.exists(text_embeddings_dir):
    ...
    all_text_embeddings = []
    text_embeddings={}
    test_dl1 = DataLoader(dataset=Clotho_dataset, batch_size=1, shuffle=False, drop_last=False,
            num_workers=4, pin_memory=True, persistent_workers=True)

    with torch.no_grad():
        # dynamic embedding classification
        for batch_idx, (x, target) in enumerate(test_dl1):
            current_text_embedding_dynamic_path=f'{parameter_embedding_folder}/text_embeddings_{int(layers[target]+1)}.pth'
...
if os.path.exists(shortlist_path):
    ...
    if show_dynamic_embedding_accuracy:
        ...
        with torch.no_grad():            
            ...
            for batch_idx, (x, target) in enumerate(test_dl):
                ...
                match_value = text_embeddings@coarse_embedding_dynamic[ModalityType.AUDIO].T 
    ...
    with torch.no_grad():
        ...
        for batch_idx, (x, target) in enumerate(test_dl):
            ...
            # print("------------------")
            # print(coarse_embedding_dynamic[ModalityType.AUDIO])
            match_value = text_embeddings@coarse_embedding_dynamic[ModalityType.AUDIO].T 
            ...
            for k, top_indices, item in zip(K_list, top_indices_list, K_caption_correct_list):
                ...
                for i,row in enumerate(top_indices):
                    ...
                    for item_r in row:
                        list_value.append(coarse_embedding_dynamic['audio'][item_r.item()])
    ...
    # # 指定CSV文件路径
    csv_file_path = f'e2e_{version}.csv'
...
batch_size=1
test_dl_final = DataLoader(dataset=Clotho_dataset, batch_size=1, shuffle=False, drop_last=False,
            num_workers=4, pin_memory=True, persistent_workers=True)
...
with torch.no_grad():
    ...
    # 获取模型参数和张量
    fine_image_embeddings_value= checkpoint['audio_embeddings']
