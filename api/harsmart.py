...
def HARsmart_origin(train=True):
    root_path = './.datasets/harsmart_origin/PreprocessedData-50hz/'
...
def HARsmart_relative(train=True):
    input_names = ['body_acc_x', 'body_acc_y', 'body_acc_z', 'body_gyro_x', 'body_gyro_y', 'body_gyro_z']
    root_path = './.datasets/UCI HAR Dataset/'
    if train:
        root_path = os.path.join(root_path, 'train')
    ...
    X = torch.stack(X, dim=1)
    # Y = torch.from_numpy(np.loadtxt(os.path.join(root_path, 'y_' + ('train' if train else 'test') + '.txt'))-1).long()
    Y=torch.tensor([i for i in range(X.shape[0])])
    dataset = data.TensorDataset(X, Y)
    return dataset

if __name__ == "__main__":
    val_dataset = HARsmart(train=True)
    val_loader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=2, shuffle=False, drop_last=False,
