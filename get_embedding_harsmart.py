...
parser.add_argument("--lora_dir", default='/home/u2021010261/data/yx/Mobile-Search-Engine-main/.checkpoints/lora/harsmart/without_head/trunk/e50/{lora_layers}',type=str, help="Number of audio blocks")
parser.add_argument("--embedding_dir", default='parameters/image/harsmart/val/without_head/embeddings_{i_block}.pth',type=str, help="Number of audio blocks")
...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    handlers=[logging.FileHandler(f'logs/embeddings/harsmart_{lora_layers}_{version}_{formatted_time}.log')], 
                    force=True)
#device = "cuda:0" if torch.cuda.is_available() else "cpu"

text_prompt = 'a photo of {}.'

model_1 = imagebind_model.imagebind_huge(pretrained=True,imu_num_blocks=lora_layers)
...
embedding_dir=embedding_dir.format(i_block=i_block)
import pathlib

os.makedirs(pathlib.Path(embedding_dir).parent,exist_ok=True)

if lora:
    model_1.modality_trunks.update(LoRA.apply_lora_modality_trunks(model_1.modality_trunks, rank=4,
                                        layer_idxs={ModalityType.IMU: [i for i in range(1,lora_layers+1)]},
                                        modality_names=[ModalityType.IMU]))
...
with torch.no_grad():
    ...
    for batch_idx, (x, target) in enumerate(test_dl):
            ...
            current_embeddings = model_1(inputs)[ModalityType.IMU]
            
            if embeddings:
                embeddings[ModalityType.IMU] = torch.cat([embeddings[ModalityType.IMU], current_embeddings], dim=0)
            else:
                embeddings[ModalityType.IMU] = current_embeddings
    ...
    embeddings[ModalityType.IMU] = embeddings[ModalityType.IMU].to(device)
    logging.info(f"embeddings computed")
    
    torch.save({
        'vision_embeddings': embeddings[ModalityType.IMU]
    }, embedding_dir)
