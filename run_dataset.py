...
if steps[0]==1:
    for i in range(1,13):
        if i%(worker_num-1)==0 and i!=0:
            command = f"python get_embedding_clotho.py --lora_layers {i} --lora_dir {lora_dir} --embedding_dir {embeddings_path} --dataset {dataset}" # stop to run this command
        else:
            command = f"python get_embedding_clotho.py --lora_layers {i}  --lora_dir {lora_dir} --embedding_dir {embeddings_path} --dataset {dataset}" # put it in the backend and run the next one concurrently
