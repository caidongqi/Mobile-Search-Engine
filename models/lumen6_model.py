...
class ImageBindModel(nn.Module):
    ...
    def _create_modality_trunks(
        ...
        def instantiate_trunk_vison(
            embed_dim, num_blocks1,num_blocks2, num_heads, pre_transformer_ln, add_bias_kv, drop_path
        ):
        ...
        modality_trunks = {}
        modality_trunks[ModalityType.VISION] = instantiate_trunk_vison(
            vision_embed_dim,
            vision_num_blocks_1,
            vision_num_blocks_2,
            vision_num_heads,
    ...
    def forward(self, inputs):
        ...
        for modality_key, modality_value in inputs.items():
            ...
            if modality_value is not None:
                ...
                #modality_value1 = self.modality_trunks[modality_key](**trunk_inputs)
               
                modality_value = self.modality_trunks[modality_key](**trunk_inputs)
                # mid_value={}
                # #mid_value['tokens']=modality_value
                # # 假设 modality_value 是一个 PyTorch 张量
                # torch.save(modality_value, 'modality_value.pt')
                # # 加载保存的向量
                # mid_value['tokens'] = torch.load('modality_value.pt')
                # modality_value = self.modality_trunks_2[modality_key](**mid_value)
